...
class ExpectedBaseHeadContractTests(unittest.TestCase):
    ...
    def test_behavioral_falsifier_is_nonvacuous(self):
        # This is intentionally a behavioral falsifier, not a source-text assertion.
        # If transition_guard.validate stops executing the state/control/assignment
        # equality check, this test loses its required error and fails.
        base = "a" * 40
        state, cp, control, ap, assignment = bound_fixture(base)
