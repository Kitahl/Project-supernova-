...
class RuntimeUpdateLineageTests(unittest.TestCase):
    ...
    def errors(self):
        return self.mod.runtime_receipt_errors(
            self.root,
            self.parent,
            self.current,
            ["foundry_sha256", "mastermind_sha256"],
        )
    ...
    def test_wrong_artifact_hash_fails(self):
        ...
        self.assertTrue(any("Foundry hash mismatch" in e for e in self.errors()))

    def test_wrong_substrate_epoch_blob_fails(self):
        r = self.receipt(); r["artifact_hashes"]["substrate_epoch_git_identity"] = "0" * 40; self.write_receipt(r)
        self.assertTrue(any("substrate epoch blob mismatch" in e for e in self.errors()))

    def test_fresh_evidence_in_replay_binding_fails(self):
        r = self.receipt(); r["fresh_prospective_evidence_refs"] = ["forbidden-fresh-ref"]; self.write_receipt(r)
        self.assertTrue(any("consumed fresh prospective evidence" in e for e in self.errors()))

    def test_failed_validator_row_fails(self):
        r = self.receipt(); r["validator_results"][0]["status"] = "FAIL"; self.write_receipt(r)
        self.assertTrue(any("validator_results[0] is not PASS" in e for e in self.errors()))

    def test_wrong_independent_qualification_class_fails(self):
        r = self.receipt(); r["independent_verification"]["qualification_class"] = "SCIENTIFIC_PROMOTION"; self.write_receipt(r)
        self.assertTrue(any("qualification class mismatch" in e for e in self.errors()))

    def test_path_traversal_is_rejected(self):
        self.current["runtime_update_receipt_path"] = "../escape.json"
        self.assertTrue(any("requires runtime/updates" in e for e in self.errors()))
