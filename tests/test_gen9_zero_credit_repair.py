...
class Gen9ZeroCreditRepairTests(unittest.TestCase):
 def test_assignment_fresh_scope_is_required_when_fresh_allowed(self):
  schema=load('schemas/assignment.schema.json')['$defs']['worker'];v=Draft202012Validator(schema)
 ...
 def test_verifier_assurance_empty_requires_explicit_transport_only_disposition(self):
  schema=load('schemas/branch_verification.schema.json')
  props=schema['properties']
  self.assertIn('verifier_assurance_disposition',schema['required'])
  self.assertIn('RUN_TIMING_UNKNOWN',schema['$defs']['liveness']['properties']['receipt_status']['enum'])
  self.assertIn('NOT_APPLICABLE_TRANSPORT_ONLY',props['verifier_assurance_disposition']['enum'])
  self.assertIn('ASSURANCE_RECORDS_PRESENT',props['verifier_assurance_disposition']['enum'])

 def test_issue_exact_failure_and_required_test_are_mandatory(self):
  issue=load('schemas/branch_report.schema.json')['$defs']['issue_record']
  self.assertIn('exact_failure',issue['required']);self.assertIn('required_test',issue['required'])

 def test_mm03_payload_is_closed(self):
  schema=load('schemas/branch_report.schema.json');rule=[r for r in schema['allOf'] if r.get('if',{}).get('properties',{}).get('worker_id',{}).get('const')=='MM03'][0]
  self.assertFalse(rule['then']['properties']['role_payload']['additionalProperties'])
