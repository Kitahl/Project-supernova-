...
11. Build the complete final report against the frozen report schema. Set `worker_auth_scheme="PS-HMAC-SHA256-CANONICAL-REPORT-2"` and the public commitment.
12. Remove `worker_auth_proof`, canonicalize the entire remaining report as sorted-key compact UTF-8 JSON (`ensure_ascii=false`), compute lowercase HMAC-SHA256 with the prompt-private secret bytes, then insert `worker_auth_proof`.
13. Write exactly one file `reports/<cohort>/<worker>.json` to the assigned `ps/work/<cohort>/<worker>` branch only. No other path may change.
14. `origin_reread_claim=false`. MM06 owns independent branch/head/blob reread and HMAC verification.
15. If a required structural CI status is absent/pending/failing, report that condition but do not reinterpret it as PASS. No protected evidence is consumed.
