...
if state:
    ...
    if not assignment.exists():
        ...
        if a:
            if a.get("cohort_id") != state.get("active_cohort_id"):
                err(assignment.relative_to(ROOT), "cohort != state active_cohort_id")
            if a.get("task_network_plan_id") != state.get("task_network_plan_id"):
                err(assignment.relative_to(ROOT), "plan ID mismatch")
            if a.get("runtime_state_id") != state.get("runtime_state_id"):
                err(assignment.relative_to(ROOT), "runtime ID mismatch")
            if set(a.get("workers", {})) != EXPECTED_WORKERS:
                err(assignment.relative_to(ROOT), "worker set mismatch")
            if a.get("network_mode") == "GITHUB_BUS_CALIBRATION":
                for wid, w in a.get("workers", {}).items():
                    if w.get("fresh_allowed") is not False:
                        err(assignment.relative_to(ROOT), f"{wid} fresh_allowed during calibration")
                    if w.get("opaque_evidence_ids") not in ([], None):
                        err(assignment.relative_to(ROOT), f"{wid} evidence assigned during calibration")
                    if w.get("private_manifest_id") is not None:
                        err(assignment.relative_to(ROOT), f"{wid} private manifest during calibration")

report_dir = ROOT / "reports"
report_paths = report_dir.rglob("*.json") if report_dir.exists() else []
for p in report_paths:
    r = load_json(p)
    if not r:
        continue
    if r.get("worker_id") not in EXPECTED_WORKERS:
        err(p.relative_to(ROOT), "unknown worker_id")
    if state and state.get("network_mode") == "GITHUB_BUS_CALIBRATION" and r.get("fresh_evidence_ids"):
        err(p.relative_to(ROOT), "fresh evidence in calibration report")
