...
def trusted_bootstrap_success(head_sha: str, base_sha: str | None = None, pr_number: int | None = None):
    """Require one completed designated bootstrap run, not merely context+shared bot.

    The bootstrap status must point to the exact GitHub Actions run that produced it;
    the run must be the designated immutable workflow, completed successfully under
    pull_request_target, and its status description must bind the exact PR/head/base.
    Ambiguous multiple successful designated runs fail closed.
    """
    if not (isinstance(base_sha, str) and HEX40.fullmatch(base_sha) and isinstance(pr_number, int) and pr_number > 0):
        return False
    ...
    for status in statuses:
        ...
        if run_obj.get("path") != BOOTSTRAP_WORKFLOW: continue
        if run_obj.get("event") != "pull_request_target": continue
        if run_obj.get("status") != "completed" or run_obj.get("conclusion") != "success": continue
        if (run_obj.get("repository") or {}).get("full_name") != REPO: continue
        if (run_obj.get("actor") or {}).get("login") != OWNER: continue
...
def pr_metadata_errors(pr: dict):
    errors = []; head = pr.get("head") or {}; base = pr.get("base") or {}
    head_ref = head.get("ref"); head_sha = head.get("sha"); head_repo = (head.get("repo") or {}).get("full_name"); user = (pr.get("user") or {}).get("login")
...
def is_ancestor(repo: pathlib.Path, ancestor: str, descendant: str):
    rc, _ = run(["git", "merge-base", "--is-ancestor", ancestor, descendant], repo); return rc == 0
...
def validate_pr(repo_root: pathlib.Path, pr: dict, trusted_errors=None):
    head = pr.get("head") or {}; base = pr.get("base") or {}; head_sha = head.get("sha"); base_sha = base.get("sha")
    ...
    try:
        ...
        for ctx, errors in results.items():
            if errors: post_status(head_sha, ctx, "failure", "FAIL " + errors[0])
                ...
                label = "PASS" if "state/CURRENT.json" in changed else "PASS/N-A non-transition"; prefix = "trusted-bootstrap-run" if authority_drift else "trusted-main"
