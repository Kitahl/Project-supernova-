...
GEN7_INVALIDATED_G = "7c182fb7ce3a3941f86f7508bbb4a18152402bb8"
GEN7_INVALIDATED_STATE_BLOB = "856481759722e23ff9a652ce140f304efe13b023"
GEN7_SUPERSESSION_PATH = "superseded/CAL-BR-007-v25-c13b6ee4.json"
AUTHORITY_PREFIXES = ("scripts/", "tests/", "schemas/", "config/", ".github/workflows/")
AUTHORITY_PATHS = {
    "PROTOCOL.md", "BRANCH_PROTOCOL.md", "BRANCH_WORKER_PROTOCOL.md", "SESSION_STANDARD.md",
    "plan/PLAN.json", "requirements-validation.lock", "branch/CONFIG.json",
    "research/open_lanes.json", "benchmark/pool_disposition.json",
}
...
def authority_path_changes(changed): return sorted(path for path in changed if path in AUTHORITY_PATHS or path.startswith(AUTHORITY_PREFIXES))
...
def trusted_bootstrap_success(head_sha, base_sha=None, pr_number=None):
    ...
    expected_desc = expected_bootstrap_description(pr_number, head_sha, base_sha); valid_run_ids = []
    for status in statuses:
        if status.get("context") != BOOTSTRAP_CONTEXT or status.get("state") != "success": continue
        if (status.get("creator") or {}).get("login") != BOOTSTRAP_CREATOR: continue
        if status.get("description") != expected_desc: continue
        match = RUN_URL_RE.fullmatch(str(status.get("target_url") or ""))
        if not match: continue
        run_id = match.group(1)
        try: run_obj = api("/actions/runs/" + run_id) or {}
        except Exception: continue
        if run_obj.get("id") != int(run_id): continue
        if run_obj.get("path") != BOOTSTRAP_WORKFLOW or run_obj.get("event") != "pull_request_target": continue
        if run_obj.get("status") != "completed" or run_obj.get("conclusion") != "success": continue
        if (run_obj.get("repository") or {}).get("full_name") != REPO or (run_obj.get("actor") or {}).get("login") != OWNER: continue
...
def pr_metadata_errors(pr):
    errors=[];head=pr.get("head") or {};base=pr.get("base") or {};head_ref=head.get("ref");head_sha=head.get("sha");head_repo=(head.get("repo") or {}).get("full_name");user=(pr.get("user") or {}).get("login")
    if base.get("ref") != "main": errors.append("PR base is not main")
    if head_repo != REPO: errors.append("PR head repository is not canonical repository")
    if user != OWNER: errors.append("PR author is not repository owner")
    if not isinstance(head_ref, str) or not head_ref.startswith(ALLOWED_HEAD_PREFIXES): errors.append("PR head prefix is not admitted")
    if not isinstance(head_sha, str) or not HEX40.fullmatch(head_sha): errors.append("PR head SHA is invalid")
    return errors
...
def changed_file_mode_errors(repo,head_sha,changed):
    ...
    for path in changed:
        ...
        if rc: errors.append("cannot inspect git mode for "+path); continue
        if not out.strip(): continue
        mode=out.split(None,1)[0]
        if mode!="100644": errors.append(f"non-regular candidate path {path} mode={mode}")
    return errors
...
def exact_invalidated_gen7_repair_parent(candidate_root, base_sha, old, changed):
    """One-shot exact-state escape from invalidated Gen7 to non-countable staging.

    This is not a generic verification bypass. It can fire only for the immutable
    historical Gen7 state blob that was invalidated by accepted control defects,
    and only when the candidate records typed zero-credit supersession and moves
    to generation 8 repair staging with streak 0 and fresh disabled.
    """
    rc,out=run(["git","rev-parse",base_sha+":state/CURRENT.json"],candidate_root)
    if rc or out.strip()!=GEN7_INVALIDATED_STATE_BLOB: return False
    if not (
        old.get("generation_seq")==7 and old.get("active_cohort_id")==GEN7_INVALIDATED_COHORT and
        old.get("generation_head_sha")==GEN7_INVALIDATED_G and old.get("calibration_countable_current") is True and
        old.get("calibration_streak")==0 and old.get("fresh_allowed_globally") is False
    ): return False
    ...
    try:
        ...
        receipt=json.loads((candidate_root/GEN7_SUPERSESSION_PATH).read_text(encoding="utf-8"))
    except Exception: return False
    if not (
        new.get("generation_seq")==8 and new.get("active_parent_state_git_identity")==GEN7_INVALIDATED_STATE_BLOB and
        new.get("active_cohort_id")!=GEN7_INVALIDATED_COHORT and new.get("calibration_countable_current") is False and
        new.get("calibration_streak")==0 and new.get("fresh_allowed_globally") is False and
        GEN7_INVALIDATED_COHORT in set(new.get("superseded_cohorts") or [])
    ): return False
    expected={
        "schema_version":"PS-COHORT-SUPERSESSION-1",
        "cohort_id":GEN7_INVALIDATED_COHORT,
        "generation_head_sha":GEN7_INVALIDATED_G,
        "state_blob_sha":GEN7_INVALIDATED_STATE_BLOB,
        "disposition":"INVALIDATED_ZERO_CREDIT_AUTHORITATIVE_CONTROL_DEFECTS",
        "calibration_credit":0,
        "fresh_evidence_consumed":False,
        "replacement_generation_seq":8,
        "replacement_countable":False,
    }
    return isinstance(receipt,dict) and receipt==expected
...
def report_admission(candidate_root, base_sha, changed):
    ...
    try:
        ...
        cohort=old["active_cohort_id"];root=candidate_root/"history"/cohort
        con=json.loads((root/"CONSOLIDATION.json").read_text(encoding="utf-8"));ver=json.loads((root/"verification.json").read_text(encoding="utf-8"));integ=json.loads((root/"integration.json").read_text(encoding="utf-8"))
...
def transition_admission(trusted_root,candidate_root,base_sha,head_sha,changed):
    if "state/CURRENT.json" not in changed: return []
    env=os.environ.copy();env["SUPERNOVA_VALIDATE_ROOT"]=str(candidate_root);env["SUPERNOVA_BASE_SHA"]=base_sha;env["SUPERNOVA_HEAD_SHA"]=head_sha;errors=[]
    for script in ("scripts/parent_lineage_guard.py","scripts/transition_guard.py"):
        ...
        if rc: errors.append(script+" failed: "+out[-1200:])
    return errors

def validate_pr(repo_root,pr,trusted_errors=None):
    head=pr.get("head") or {};base=pr.get("base") or {};head_sha=head.get("sha");base_sha=base.get("sha");metadata_errors=pr_metadata_errors(pr)
    if metadata_errors:
        if isinstance(head_sha,str) and HEX40.fullmatch(head_sha): fail_contexts(head_sha,"trusted admission refused: "+metadata_errors[0])
        return
    ...
    changed=changed_files(repo_root,trusted,head_sha);authority_drift=authority_path_changes(changed)
    if authority_drift and not trusted_bootstrap_success(head_sha,base_sha,number): fail_contexts(head_sha,"trusted admission refused: authority bytes changed without source-verified bootstrap: "+authority_drift[0]); return
    mode_errors=changed_file_mode_errors(repo_root,head_sha,changed)
    if mode_errors: fail_contexts(head_sha,"trusted admission refused: "+mode_errors[0]); return
    ...
    try:
        ...
        for ctx,errs in results.items():
            if errs: post_status(head_sha,ctx,"failure","FAIL "+errs[0])
                ...
                label="PASS" if "state/CURRENT.json" in changed else "PASS/N-A non-transition";prefix="trusted-bootstrap-run" if authority_drift else "trusted-main";post_status(head_sha,ctx,"success",prefix+" exact-head "+label)
    finally:
