...
BOOTSTRAP_WORKFLOW_PATH = ".github/workflows/supernova-authority-bootstrap.yml"
...
def _open_pr_for_head(head_sha: str):
    try:
        prs = api("/commits/" + head_sha + "/pulls?per_page=20") or []
    except Exception:
        return None
    matches = []
    for pr in prs:
        ...
        if head.get("sha") != head_sha: continue
        if (head.get("repo") or {}).get("full_name") != REPO: continue
        if base.get("ref") != "main": continue
        if (pr.get("user") or {}).get("login") != OWNER: continue
        matches.append(pr)
    return matches[0] if len(matches) == 1 else None


def _run_proves_designated_bootstrap(run_id: int, pr: dict, head_sha: str):
    try:
        run_obj = api(f"/actions/runs/{run_id}") or {}
        workflow_id = run_obj.get("workflow_id")
        if not isinstance(workflow_id, int): return False
        workflow = api(f"/actions/workflows/{workflow_id}") or {}
    except Exception:
        return False
    base_sha = (pr.get("base") or {}).get("sha")
    expected_title = f"Supernova Authority Bootstrap PR #{pr.get('number')} HEAD={head_sha} BASE={base_sha}"
    return (
        run_obj.get("status") == "completed"
        and run_obj.get("conclusion") == "success"
        and run_obj.get("event") == "pull_request_target"
        and (run_obj.get("repository") or {}).get("full_name") == REPO
        and workflow.get("path") == BOOTSTRAP_WORKFLOW_PATH
        and run_obj.get("display_title") == expected_title
    )


def trusted_bootstrap_success(head_sha: str):
    """Require exact designated workflow-run provenance, not context text alone."""
    pr = _open_pr_for_head(head_sha)
    if pr is None:
        return False
    try:
    ...
    for status in statuses:
        ...
        m = RUN_URL.fullmatch(target)
        if not m or m.group(1) != REPO:
            continue
        run_id = int(m.group(2))
        if _run_proves_designated_bootstrap(run_id, pr, head_sha):
            valid.append(run_id)
...
def pr_metadata_errors(pr: dict):
    ...
    head_repo = (head.get("repo") or {}).get("full_name"); user = (pr.get("user") or {}).get("login")
...
def trusted_self_check(trusted_root: pathlib.Path):
    errors = []
    for cmd in ([sys.executable, "scripts/validate_bus.py"], [sys.executable, "-m", "unittest", "discover", "-s", "tests", "-p", "test_*.py"]):
        rc, out = run(cmd, trusted_root)
        if rc: errors.append("trusted main self-check failed: " + " ".join(cmd) + ": " + out[-1200:])
    return errors
...
def exact_noncountable_gen6_bootstrap_parent(candidate_root: pathlib.Path, base_sha: str, old: dict):
    ...
    if rc or out.strip() != GEN6_BOOTSTRAP_STATE_BLOB: return False
    return (
        old.get("generation_seq") == 6 and old.get("active_cohort_id") == GEN6_BOOTSTRAP_COHORT
        and old.get("calibration_countable_current") is False and old.get("calibration_streak") == 0
        and old.get("fresh_allowed_globally") is False and old.get("repo_policy_status") == "UNVERIFIED_BLOCKING"
...
def transition_admission(trusted_root: pathlib.Path, candidate_root: pathlib.Path, base_sha: str, head_sha: str, changed: list[str]):
    if "state/CURRENT.json" not in changed: return []
    env = os.environ.copy(); env["SUPERNOVA_VALIDATE_ROOT"] = str(candidate_root); env["SUPERNOVA_BASE_SHA"] = base_sha; env["SUPERNOVA_HEAD_SHA"] = head_sha
    errors = []
...
def validate_pr(repo_root: pathlib.Path, pr: dict, trusted_errors=None):
    head = pr.get("head") or {}; head_sha = head.get("sha")
    ...
    if metadata_errors:
        ...
        return
    if trusted_errors:
        fail_contexts(head_sha, trusted_errors[0]); return
    number = pr["number"]; trusted = trusted_main_sha(repo_root)
    run(["git", "fetch", "--no-tags", "origin", f"pull/{number}/head"], repo_root)
    if not is_ancestor(repo_root, trusted, head_sha):
        fail_contexts(head_sha, "trusted admission refused: PR head does not descend from exact current main"); return
    ...
    if authority_drift and not trusted_bootstrap_success(head_sha):
        fail_contexts(head_sha, "trusted admission refused: authority bytes changed without source-verified bootstrap: " + authority_drift[0]); return
    mode_errors = changed_file_mode_errors(repo_root, head_sha, changed)
    if mode_errors:
        fail_contexts(head_sha, "trusted admission refused: " + mode_errors[0]); return
    ...
    try:
        ...
        if rc:
            fail_contexts(head_sha, "trusted admission could not create candidate data worktree"); return
        ...
        results = {"supernova/static-control": static_errors, "supernova/report-admission": report_errors, "supernova/transition-admission": transition_errors}
        for ctx, errors in results.items():
            if errors: post_status(head_sha, ctx, "failure", "FAIL " + errors[0])
            else:
...
def main():
    root = pathlib.Path.cwd().resolve(); trusted_errors = trusted_self_check(root)
