...
import strict_json
from scheduler_admission_guard import validate_scheduler_admission
...
MINIMUM_HARDENED_CONTROL={
 ...
 'config/protocol_freeze.json','config/repo_policy.json','config/roles.json','config/worker_auth.json','config/task_registry_v25.json','config/task_registry_semantics_v25.json','config/checker_pins.json','config/countable_control_set_v25.json','config/admission_authority.json','config/root_tcb_epoch_v25.json','config/root_epoch10_scheduler_admission_seed_v25.json','config/root_epoch10_scheduler_admission_epoch_v25.json','branch/CONFIG.json','research/open_lanes.json','benchmark/registry.json','benchmark/pool_disposition.json',
 'schemas/state.schema.json','schemas/control.schema.json','schemas/assignment.schema.json','schemas/branch_report.schema.json','schemas/branch_verification.schema.json','schemas/branch_integration.schema.json','schemas/branch_director.schema.json','schemas/branch_consolidation.schema.json','schemas/lane_liveness_observation.schema.json','schemas/cohort_liveness_contract.schema.json','schemas/verifier_assurance.schema.json','schemas/runtime_update.schema.json','schemas/private_manifest_contract.schema.json','schemas/scheduler_manifest.schema.json','schemas/preactivation_receipt.schema.json','schemas/scheduler_admission.schema.json',
 'scripts/strict_json.py','scripts/validate_bus.py','scripts/validate_branch_bus_v251.py','scripts/parent_lineage_guard.py','scripts/transition_guard.py','scripts/generation_delta_guard.py','scripts/scheduler_admission_guard.py','scripts/reconcile_branch_rest.py','scripts/reconcile_branch_statuses.py','scripts/reconcile_v25_admission.py','scripts/reconcile_open_prs.py','scripts/check_lane_liveness.py','scripts/liveness_contract_guard.py',
 'tests/test_v25_report_contracts.py','tests/test_source_bound_repo_policy.py','tests/test_countable_control_freeze.py','tests/test_countable_control_gate_consistency.py','tests/test_generation_delta_policy.py','tests/test_root_epoch10_scheduler_admission.py','tests/test_scheduler_admission_negative.py','tests/test_actions_trigger_bridge.py','tests/test_open_pr_admission_trust.py','tests/test_privileged_admission_workflows.py','tests/liveness/test_liveness_monitor.py','tests/verifier_assurance/test_verifier_assurance_schema.py',
 '.github/workflows/supernova-v25-admission.yml','.github/workflows/supernova-rest-branch-reconciler.yml','.github/workflows/supernova-open-pr-reconciler.yml','.github/workflows/supernova-actions-heartbeat.yml','.github/workflows/supernova-comment-admission.yml','.github/workflows/supernova-pr-target-admission.yml','.github/workflows/supernova-root-epoch10-scheduler-admission-seed.yml','.github/workflows/supernova-liveness-monitor.yml','requirements-validation.lock'
}


def req(path,method='GET',data=None):
 payload=None if data is None else strict_json.canonical_dumps(data).encode('utf-8')
 r=urllib.request.Request(API+path,data=payload,method=method);r.add_header('Accept','application/vnd.github+json');r.add_header('X-GitHub-Api-Version','2022-11-28')
...
def source_bound_scheduler_admission(cohort,admission):
 """The copied scheduler admission must be byte-identical to exact MM06 preactivation evidence."""
 ...
 try:
  ...
  copy=dict(admission);source_copy=dict(source)
  if copy!=source_copy:e.append('scheduler admission copy differs from MM06 preactivation source')
...
def generation_check(state):
 ...
 try:
  ...
  if countable:
   _,accepted_contract=content('config/countable_control_set_v25.json','main');required_contract=required_countable_paths(accepted_contract);missing=sorted(required_contract-required)
...
def main():
 ...
 G=state['generation_head_sha'];ge=generation_check(state);status(G,'supernova/static-control','failure' if ge else 'success',('FAIL '+ge[0]) if ge else 'v2.5 frozen static control PASS')
