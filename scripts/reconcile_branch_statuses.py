...
def validate(branch,generation_head):
 ...
 p=subprocess.run([sys.executable,'scripts/validate_branch_bus_v252.py','--branch',branch,'--generation-head',generation_head],cwd=ROOT,text=True,stdout=subprocess.PIPE,stderr=subprocess.STDOUT,check=False)
