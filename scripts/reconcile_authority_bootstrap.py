...
ALLOWED_EXACT = {
    "PROTOCOL.md", "WORKER_PROTOCOL.md", "BRANCH_PROTOCOL.md", "BRANCH_WORKER_PROTOCOL.md",
    "SESSION_STANDARD.md", "plan/PLAN.json", "requirements-validation.lock", "branch/CONFIG.json",
}
FORBIDDEN_EXACT = {
    "state/CURRENT.json", "config/worker_auth.json", "config/task_registry_v25.json",
    "benchmark/registry.json", "benchmark/pool_disposition.json", "research/open_lanes.json",
}
FORBIDDEN_PREFIXES = (
    "state/", "control/", "assignments/", "reports/", "verification/", "integration/",
    "history/", "transitions/", "superseded/", "benchmark/", "research/",
)

# Compatibility name retained for the original regression. Epoch 2 expands this
# exact static root through bootstrap_root_paths() using accepted-main metadata.
...
ROOT_EPOCH2_STATIC_PATHS = ROOT_BOOTSTRAP_PATHS | {
...
REQUIRED_INSTALLED_CONTROL_PATHS = {
    "config/admission_authority.json", "config/authority_bootstrap_v25.json", "config/root_tcb_epoch_v25.json",
    "config/root_rotation_seed_v25.json", "config/substrate_epoch_v25.json", "config/read_only_probe_parallelism_v25.json",
    "scripts/reconcile_open_prs.py", "scripts/reconcile_authority_bootstrap.py", "scripts/reconcile_root_rotation_seed.py",
    "tests/test_authority_bootstrap.py", "tests/test_bootstrap_root_tcb_and_head_binding.py",
    "tests/test_bootstrap_status_provenance.py", ".github/workflows/supernova-authority-bootstrap.yml",
    ".github/workflows/supernova-bootstrap-completion-reconcile.yml", ".github/workflows/supernova-root-rotation-seed.yml",
}
...
def bootstrap_root_paths(trusted_root: pathlib.Path):
    """Derive the accepted-main transitive write-capable admission TCB."""
    roots = set(ROOT_EPOCH2_STATIC_PATHS)
    admission = load_json(trusted_root, "config/admission_authority.json")
    for key in (
        "authority_bootstrap_policy", "trusted_reconciler", "trusted_authority_bootstrap_reconciler",
        "candidate_diagnostic_workflow", "bootstrap_completion_workflow", "root_tcb_epoch_path",
    ):
        value = admission.get(key)
        if isinstance(value, str) and value:
            roots.add(value)
    for key in ("trusted_validator_entrypoints", "trusted_authority_helpers", "authoritative_status_workflows"):
...
def workflow_declares_status_write(candidate_root: pathlib.Path, path: str) -> bool:
    if not path.startswith(".github/workflows/") or not (candidate_root / path).is_file():
        return False
    text = (candidate_root / path).read_text(encoding="utf-8")
    return bool(re.search(r"(?m)^\s*statuses:\s*write\s*$", text))
...
def bootstrap_invariant_errors(trusted_root: pathlib.Path, candidate_root: pathlib.Path, changed: list[str]):
    ...
    try:
        ...
        required_bootstrap = {
            "protocol_version": "2.5", "task_network_plan_id": PLAN, "enabled_after_install": True,
            "bootstrap_context": BOOTSTRAP_CONTEXT, "required_status_creator": BOOTSTRAP_CREATOR,
            "trusted_executable_source": "EXACT_ACCEPTED_MAIN", "candidate_bytes_in_privileged_phase": "DATA_ONLY",
            "candidate_diagnostics": "READ_ONLY_SEPARATE_JOB_REQUIRED", "same_repository_required": True,
            "owner_authored_required": True, "base_branch_required": "main", "exact_current_main_ancestor_required": True,
            "calibration_streak_required": 0, "fresh_allowed_globally_required": False,
            "worker_auth_change": "FORBIDDEN_IN_AUTOMATED_BOOTSTRAP",
            "state_or_scientific_change": "FORBIDDEN_IN_AUTOMATED_BOOTSTRAP",
            "bootstrap_status_provenance": "DESIGNATED_WORKFLOW_RUN_EXACT_PR_BINDING_REQUIRED",
            "designated_workflow_path": ".github/workflows/supernova-authority-bootstrap.yml",
            "completion_workflow_path": ".github/workflows/supernova-bootstrap-completion-reconcile.yml",
            "root_tcb_epoch_path": "config/root_tcb_epoch_v25.json",
            "new_status_writing_workflow_requires_root_rotation": True,
            "merge_authority": "EXISTING_GITHUB_RULESET_ONLY", "bootstrap_verifier_may_bypass_ruleset": False,
            "bootstrap_verifier_may_merge": False, "failure_semantics": "FAIL_CLOSED",
        }
...
def main():
    ...
    head_repo = (head.get("repo") or {}).get("full_name"); author = (pr.get("user") or {}).get("login")
    ...
    if __name__ != "__main__" and (not diagnosed_head or not diagnosed_base):
        # Non-authoritative structural diagnostic replay imports this module and
        # monkey-patches post(); it may replay against current PR metadata only.
        diagnosed_head = head_sha
        diagnosed_base = base.get("sha")
    binding_errors = diagnostic_binding_errors(pr, diagnosed_head, diagnosed_base)
    ...
    if diagnosed_base != trusted: return fail(head_sha, "diagnosed base SHA is not exact accepted main")
    ...
    try: root_tcb = bootstrap_root_paths(root)
    except Exception as exc: return fail(head_sha, "bootstrap root TCB derivation failed: " + repr(exc))
    for path in changed:
        ...
        if path in root_tcb: return fail(head_sha, "bootstrap root self-modification requires independent seed: " + path)
    ...
    try:
        ...
        for path in changed:
            if path.startswith(".github/workflows/") and workflow_declares_status_write(tmp, path):
                return fail(head_sha, "new or changed status-writing workflow requires independent root rotation: " + path)
    ...
    post("success", head_sha, "trusted-main authority bootstrap PASS; diagnostics bound to exact head/base/run")
