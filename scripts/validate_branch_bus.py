...
AUTH_SCHEME="PS-HMAC-SHA256-CANONICAL-REPORT-2"
MODEL="GPT-5.6 Sol"; EFFORT="EXTRA_HIGH"
...
def schema(name):
 ...
 return s
def errors(o,s):return list(Draft202012Validator(s).iter_errors(o)) if o is not None and s is not None else []
def validate(o,s,p):
 for x in sorted(errors(o,s),key=lambda z:list(z.path)):
...
def session_mismatches(r,a,w):
 h=(r or {}).get("session_header",{}); aw=(a or {}).get("workers",{}).get(w,{})
 expected={"session_name":SESSION_NAMES.get(w),"target_program":aw.get("target_program"),"phase":(a or {}).get("phase"),"iteration_id":(a or {}).get("cohort_id"),"iteration_number":(a or {}).get("generation_seq"),"role_id":w,"goal":aw.get("goal"),"plan_id":PLAN,"runtime_state_id":(a or {}).get("runtime_state_id"),"model_target":MODEL,"reasoning_effort_target":EFFORT}
 out=[f"session_header.{k}" for k,v in expected.items() if h.get(k)!=v]
...
ref=args.branch or os.environ.get("GITHUB_REF_NAME","");kind,cohort,worker=context(ref);cohort=args.cohort or cohort
if not cohort:err("branch",f"cannot derive cohort from {ref!r}")
for p in ROOT.rglob("*.json"):
 if ".git" not in p.parts:
  o=load(p)
  if o is not None:walk_public(o,str(p.relative_to(ROOT)))
S={n:schema(n) for n in ["control","assignment","report","verification","integration","director","research","benchmark_registry","benchmark_completion","private_manifest_contract","branch_generation","branch_consolidation"]}
...
if (config or {}).get("overlay_id")!=OVERLAY or (config or {}).get("base_task_network_plan_id")!=PLAN:err("branch/CONFIG.json","overlay/base-plan mismatch")
if set((config or {}).get("overlay_control_files",[]))!=OVERLAY_CONTROL:err("branch/CONFIG.json","overlay control set mismatch")
if (config or {}).get("base_control_file_count")!=21 or (config or {}).get("overlay_control_file_count")!=6 or (config or {}).get("branch_control_file_count")!=27:err("branch/CONFIG.json","declared control counts mismatch")
if (config or {}).get("worker_auth_scheme")!=AUTH_SCHEME:err("branch/CONFIG.json","worker auth scheme mismatch")
if set((auth or {}).get("commitments",{}))!=WORKERS:err("config/worker_auth.json","worker commitment set mismatch")
validate(registry,S["benchmark_registry"],"benchmark/registry.json")

if cohort:
 cp=ROOT/"control"/f"{cohort}.json";apath=ROOT/"assignments"/f"{cohort}.json";c=load(cp) if cp.exists() else None;a=load(apath) if apath.exists() else None
 validate(c,S["control"],str(cp.relative_to(ROOT)) if cp.exists() else "control")
 validate(a,S["assignment"],str(apath.relative_to(ROOT)) if apath.exists() else "assignment")
 ...
 if c:
  ...
  if set(c.get("files",{}))!=CONTROL:err(str(cp.relative_to(ROOT)),f"frozen control set must be exact 21+6 union; got {len(c.get('files',{}))}")
 ...
 if a:
  if a.get("task_network_plan_id")!=PLAN or a.get("cohort_id")!=cohort:err(str(apath.relative_to(ROOT)),"plan/cohort mismatch")
  if set(a.get("workers",{}))!=WORKERS:err(str(apath.relative_to(ROOT)),"worker set mismatch")
  if c and (a.get("control_manifest_path")!=str(cp.relative_to(ROOT)) or a.get("control_manifest_git_identity")!=blob(cp)):err(str(apath.relative_to(ROOT)),"control binding mismatch")
  if a.get("network_mode")=="GITHUB_BUS_CALIBRATION":
   for w,x in a.get("workers",{}).items():
    if x.get("fresh_allowed") is not False or x.get("opaque_evidence_ids")!=[] or x.get("private_manifest_id") is not None or x.get("private_manifest_git_identity") is not None:err(str(apath.relative_to(ROOT)),f"{w} not replay-only")
 if kind=="worker" and worker:
  ...
  if not r:err(str(rp.relative_to(ROOT)),"worker report missing")
   ...
   validate(r,S["report"],str(rp.relative_to(ROOT)))
   expected={"task_network_plan_id":PLAN,"cohort_id":cohort,"worker_id":worker,"assignment_id":a.get("assignment_id") if a else None,"assignment_git_identity":blob(apath) if a else None,"control_manifest_id":a.get("control_manifest_id") if a else None,"control_manifest_git_identity":blob(cp) if c else None,"network_checkpoint_id":a.get("network_checkpoint_id") if a else None,"runtime_state_id":a.get("runtime_state_id") if a else None,"visibility_token":a.get("workers",{}).get(worker,{}).get("visibility_token") if a else None,"worker_auth_scheme":AUTH_SCHEME,"worker_auth_commitment":(auth or {}).get("commitments",{}).get(worker),"status":"VALID_ASSIGNED_REPORT","public_safety_status":"PASS","origin_reread_claim":False}
   for k,v in expected.items():
    if r.get(k)!=v:err(str(rp.relative_to(ROOT)),f"{k} mismatch")
   for x in session_mismatches(r,a,worker):err(str(rp.relative_to(ROOT)),f"strict session mismatch {x}")
   ...
   # Direct negative probes: schema closure and strict session equality.
   miss=copy.deepcopy(r);miss.pop("task_network_plan_id",None)
   if not errors(miss,S["report"]):err(str(rp.relative_to(ROOT)),"negative probe failed: missing required field accepted")
   extra=copy.deepcopy(r);extra["unexpected_top_level"]="x"
   if not errors(extra,S["report"]):err(str(rp.relative_to(ROOT)),"negative probe failed: undeclared field accepted")
   wrong=copy.deepcopy(r);wrong["session_header"]["phase"]="__WRONG_PHASE__"
   if not session_mismatches(wrong,a,worker):err(str(rp.relative_to(ROOT)),"negative probe failed: wrong phase accepted")
 if kind=="verify":
  p=ROOT/"verification"/f"{cohort}.json"
  if p.exists():validate(load(p),S["verification"],str(p.relative_to(ROOT)))
 if kind=="integrate":
  p=ROOT/"integration"/f"{cohort}.json"
  if p.exists():validate(load(p),S["integration"],str(p.relative_to(ROOT)))
 if kind=="consolidate":
  p=ROOT/"history"/cohort/"CONSOLIDATION.json"
  if p.exists():validate(load(p),S["branch_consolidation"],str(p.relative_to(ROOT)))

if E:
 print("BRANCH BUS VALIDATION FAILED")
 for x in E:print("-",x)
 sys.exit(1)
print(f"BRANCH BUS VALIDATION PASS ref={ref} cohort={cohort} kind={kind} auth={AUTH_SCHEME}")
